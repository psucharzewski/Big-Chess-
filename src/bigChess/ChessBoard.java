...
public class ChessBoard extends GridPane
{
    ...
    //public Space[][] spacesWhite = new Space[2][8];
    //public Space[][] spacesBlack = new Space[2][8];
    // const
    ...
    public void setActiveSpace(Space s)
    {
        ...
        ///if (this.activeSpace != null) if(this.activeSpace.getPiece().color != player) return;
    ...
    protected boolean processMove(MoveInfo p)
    {
        if (moveIsValid(p))
        {
            ...
            this.pawnPromotion(spaces[p.getNewX()][p.getNewY()]);
            return true;
        }
        else // invalid move
        {
    ...
    public boolean moveIsValid(MoveInfo p)
    {
        ...
        {//iterates through multiple times if has multiple possible moves
            if(activeSpace.getPiece().color != player) break;
        	multiMoveCount = 1;
    ...
    public boolean pawnPromotion(Space space)
    {
    	if((space.getPieceColor()=="white") && space.getY()==7 && space.getPiece().getName()=="pawn") {
    		space.setPiece(new Queen(true));
    		return true;
    	}
    	else if((space.getPieceColor()=="black") && space.getY()==0 && space.getPiece().getName()=="pawn") {
    		space.setPiece(new Queen(false));
    		return true;
    	}
    	return false;
